...
const SelfCare = () => {
  ...
  // Automatically expand if user navigates to /SelfCare
  useEffect(() => {
    if (location.pathname === "/SelfCare") {
      setShowAll(true);
    }
  }, [location]);

  
  const displayedProducts = showAll ? PersonalCare : PersonalCare.slice(0, 5);
  ...
  return (
    <div className="container mx-auto py-9">
      ...
      {/* View More Button */}
      {PersonalCare.length > 5 && !showAll && location.pathname !== "/SelfCare" && (
        <div className="text-center mt-4">
          <button
            ...
            onClick={() => navigate("/SelfCare")} // Use navigate() instead of window.location.href
          >
            View More
          </button>
        </div>
